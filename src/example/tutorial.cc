...
void load_joml_from_file() {
    std::string err_msg;
    cloris::Config* conf = cloris::Config::instance()->Load("../conf/common.ini", SRC_LOCAL | FMT_JOML | CMT_SHARP, &err_msg);
    if (conf) {
...
void test_zookeeper() {
    ...
    cloris::ConfNode* node = conf->GetConfNode("rules/splash");
    if (node) {
        for (cloris::ConfNode::ChildrenIterator iter = node->begin(); iter != node->end(); ++iter) {
